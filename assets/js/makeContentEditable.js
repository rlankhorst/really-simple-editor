...
function addEditableListener(identifier) {
    ...
    if (identifier === 'main') {
        ...
        const mainButtons = document.querySelector('.rsed_editButtons_main');
        if (mainButtons) {
            editButtons['main'] = mainButtons;
        } else {
            editButtons['main'] = document.querySelector('.rsed_noShortCode');
        }

    } else if (identifier === 'title') {
        context = 'title';
        editableContent['title'] = document.querySelectorAll('.rsed_title');
    ...
    // title can be more than one all need event listener, since we can't discern between them
    if (identifier === 'title') {
        for (let el of editableContent['title']) {
            editableContent[context] = el;
            el.addEventListener('click', editOnClickEvent);
        }
        return;
    }
    ...
    function editOnClickEvent(event) {
        ...
        if (editButtons[context].classList.contains('rsed_noShortCode')) {
...
function closeEditMode(event) {
    ...
    if (focus !== undefined && focus.indexOf('meta_') > -1) {
        const focusNr = focus.substr(5, 6);
        try {
...
function checkIfPlaceholdersNeeded() {
    ...
    if (!($(`.rsed_title_of_post`).text())) {
        $(`.rsed_title_of_post`).text(window.rsed_translateStrings.edit_title);
        $(`.rsed_title_of_post`).addClass('rsed_textPlaceholder');
    }
